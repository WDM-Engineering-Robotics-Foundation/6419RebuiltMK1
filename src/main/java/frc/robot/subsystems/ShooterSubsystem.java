...
public class ShooterSubsystem extends SubsystemBase {
    ...
    private TalonFX shooterLeft, shooterRight, feederMotor;
    ...
    ShooterSubsystem() {
        ...
        feederMotor = new TalonFX(ShooterConstants.FEEDER_MOTOR_ID); {
            ...
            outConfig.Inverted = ShooterConstants.FEEDER_REVERSED ? InvertedValue.Clockwise_Positive : InvertedValue.CounterClockwise_Positive;
        }
    ...
    public Command feedShooter() {
        return Commands.parallel(
            Commands.run(() -> {
                shooterLeft.setControl(new VelocityVoltage(ShooterConstants.BASE_VELOCITY).withFeedForward(ShooterConstants.VELO_FF));
                shooterRight.setControl(new Follower(ShooterConstants.LEFT_SHOOTER_MOTOR_ID, MotorAlignmentValue.Opposed));
            }, this),
            Commands.waitUntil(() -> {
                return shooterLeft.getClosedLoopError(true).getValueAsDouble() < ShooterConstants.VELO_TOLERANCE;
            }).andThen(Commands.run(()->{
                feederMotor.setControl(new DutyCycleOut(ShooterConstants.FEEDER_DUTY_CYCLE));
            }))
        );
    }
}
